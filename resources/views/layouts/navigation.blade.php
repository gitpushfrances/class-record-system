<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="flex items-center shrink-0">
                    <a href="@if(auth()->user()->role === 'super_admin'){{ route('admin.dashboard') }}@elseif(auth()->user()->role === 'dean'){{ route('dean.dashboard') }}@else{{ route('teacher.dashboard') }}@endif">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                    ...
                    @if(auth()->user()->role === 'dean')
                        ...
                        <x-nav-link :href="route('dean.teachers.pending')" :active="request()->routeIs('dean.teachers.*')">
                            {{ __('Pending Teachers') }}
                        </x-nav-link>
                        <x-nav-link :href="route('dean.sections.index')" :active="request()->routeIs('dean.sections.*')">
                            {{ __('Sections') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(auth()->user()->role === 'dean')
                <x-responsive-nav-link :href="route('dean.dashboard')" :active="request()->routeIs('dean.dashboard')">
                    {{ __('Dashboard') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('dean.teachers.pending')" :active="request()->routeIs('dean.teachers.*')">
                    {{ __('Pending Teachers') }}
                </x-responsive-nav-link>
                <x-responsive-nav-link :href="route('dean.sections.index')" :active="request()->routeIs('dean.sections.*')">
                    {{ __('Sections') }}
                </x-responsive-nav-link>
